...
# запишемо регулярний вираз у змінну tel_pat
tel_pat = re.compile(r'(?<![\w\d])(?:\+?(?:38)?)(?:0[\d]{9})(?=\b)')
...
# перевіряємо на введення команди "add", якщо абонент вже є - повідомимо про це, якщо ні - добавимо в книгу
    if comand == 'add':
        # перевіряємо чи номер валідний номер телефону за допомогою регулярного виразу
        if (re.match(tel_pat, tel)):
